...
case "$1" in
  webserver)
    airflow db init
    airflow variables set local_test true
    airflow users create --role Admin --username ${INITIAL_USER} --email ${INITIAL_EMAIL} --firstname ${INITIAL_FIRSTNAME} --lastname ${INITIAL_LASTNAME} --password ${INITIAL_PASSWORD}
